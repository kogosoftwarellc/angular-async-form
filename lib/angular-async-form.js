...
.directive('afControlGroup', function() {
  ...
  function controller() {
    ...
    ctrlGroup.init = function(afSubmit) {
      if (!_control) {
        throw new Error('no afControl was found in this afControlGroup');
      }

      _afSubmit = afSubmit;
      afSubmit.addControlGroup(_controlName, ctrlGroup);
    };
...
.directive('afControl', function() {
  ...
  function link(scope, el, attrs, ctrls) {
    ...
    controlGroup.setControl(attrs.name, ngModel);
  }
});
